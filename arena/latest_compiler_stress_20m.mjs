...
const durationMs=Number(args['duration-ms']??1200000),delayMs=Number(args['delay-ms']??20),seedInitial=Number(args.seed??260820);
...
function generate(id){
 const family=pick(['P1_LEXICAL','P2_PARSER','P3_AST','P4_EMITTER','P5_RUNTIME']);const a=integer(1,1000),b=integer(1,1000),c=integer(1,30);
 if(family==='P1_LEXICAL'){
  const o=pick(['+','-','*','/','%','<<','>>','&','|','^','<','>','<=','>=','==','!=']);const val=op(a,o,b);return{id,family,name:'single_operator',operator:o,source:src(typeof val==='boolean'?'bool':'num',`return ${a}${o}${b}`),shouldReject:!supported.has(o),...expected(val)};
 }
 if(family==='P2_PARSER'){
  const kind=pick(['sub_chain','div_chain','mul_precedence','paren_precedence','mixed_add_sub','spaced_div','ternary']);
 ...
 if(family==='P4_EMITTER'){
  ...
  if(kind==='modulo_raw')return{id,family,name:kind,operator:'%',source:src('num',`return ${a}%${b}`),shouldReject:true,...expected(a%b)};
  if(kind==='shift_raw')return{id,family,name:kind,operator:'<<',source:src('num',`return ${a}<<${c}`),shouldReject:true,...expected(a<<c)};
  if(kind==='bitwise_raw')return{id,family,name:kind,operator:'&',source:src('num',`return ${a}&${b}`),shouldReject:true,...expected(a&b)};
