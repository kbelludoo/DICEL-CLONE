...
function generate(id){
 ...
 if(family==='P2_PARSER'){
  ...
  return{id,family,name:kind,operator:'?:',source:src('num',`return ${a}>${b}?${a}:${b}`),shouldReject:false,...expected(a>b?a:b)};
 }
 if(family==='P3_AST'){
...
while(Date.now()-start<durationMs&&(maxProbes<=0||id<maxProbes)){id++;const p=generate(id),r=await compile(p),cat=classify(p,r);families[p.family]=(families[p.family]??0)+1;categories[cat]=(categories[cat]??0)+1;phases[r.phase]=(phases[r.phase]??0)+1;operators[p.operator]??={probes:0,pass:0,fail:0,runtime_fail:0};operators[p.operator].probes++;if(r.compile){compilePass++;operators[p.operator].pass++}else{compileFail++;operators[p.operator].fail++}if(r.runtime!==undefined){runtimeTests++;if(r.runtime_equal)runtimePass++;else{runtimeFail++;operators[p.operator].runtime_fail++}}
